...
class RouterProvisioningServiceTest extends TestCase
{
    public function test_provision_access_uses_safe_hotspot_credentials_without_creating_bypass_binding()
    {
        ...
        $mock->shouldReceive('query')
            ...
            ->with([
                ...
                '=user=hs_aabbccddeeff',
                '=password=hs_aabbccddeeff_pw',
                '=ip=192.168.88.10',
    ...
    public function test_provision_access_resolves_ip_before_creating_speed_limit_queue()
    {
        ...
        $mock->shouldReceive('query')
            ...
            ->withArgs(function ($query) {
                return is_array($query)
                    && $query[0] === '/ip/hotspot/user/add'
                    && in_array('=name=hs_112233445566', $query)
                    && in_array('=password=hs_112233445566_pw', $query)
        ...
        $mock->shouldReceive('query')
            ...
            ->with([
                '/ip/hotspot/active/login',
                '=user=hs_112233445566',
                '=password=hs_112233445566_pw',
                '=ip=192.168.88.25',
    ...
    public function test_provision_access_fails_when_auto_login_does_not_create_active_session()
    {
        ...
        $mock->shouldReceive('query')->once()->with(['/ip/hotspot/user/print', '?name=78:62:56:C5:52:61'])->andReturnSelf();
        $mock->shouldReceive('read')->once()->andReturn([]);

        $mock->shouldReceive('query')->once()->with(['/ip/hotspot/user/print', '?name=hs_786256c55261'])->andReturnSelf();
        $mock->shouldReceive('read')->once()->andReturn([]);

        $mock->shouldReceive('query')->once()->with(['/ip/hotspot/ip-binding/print', '?mac-address=78:62:56:C5:52:61'])->andReturnSelf();
        $mock->shouldReceive('read')->once()->andReturn([]);

        $mock->shouldReceive('query')->once()->with(['/queue/simple/print', '?name=RateLimit_78:62:56:C5:52:61'])->andReturnSelf();
        $mock->shouldReceive('read')->once()->andReturn([]);

        $mock->shouldReceive('query')
            ->once()
            ->withArgs(fn ($query) => is_array($query) && $query[0] === '/ip/hotspot/user/add' && in_array('=name=hs_786256c55261', $query))
            ->andReturnSelf();
        $mock->shouldReceive('read')->once()->andReturn([]);
        ...
        $mock->shouldReceive('query')
            ...
            ->with([
                '/ip/hotspot/active/login',
                '=user=hs_786256c55261',
                '=password=hs_786256c55261_pw',
                '=ip=192.168.88.233',
                '=mac-address=78:62:56:C5:52:61',
            ])
            ->andReturnSelf();
        $mock->shouldReceive('read')->once()->andReturn([]);
        ...
        $mock->shouldReceive('query')
            ->never()
            ->withArgs(fn ($query) => is_array($query) && ($query[0] ?? null) === '/queue/simple/add');
