...
<div class="space-y-4">
    <div class="bg-white border border-gray-300 shadow-sm rounded-sm p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
            ...
            <p class="text-xs text-gray-500 mt-1">Live router health, uptime, interfaces, and maintenance actions.</p>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div class="bg-white border border-gray-300 shadow-sm rounded-sm p-3">
            ...
            <span data-router-status class="mt-2 inline-flex items-center text-xs font-bold px-2 py-0.5 rounded border {{ $router['online'] ? 'text-green-700 bg-green-50 border-green-200' : 'text-red-700 bg-red-50 border-red-200' }}">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function renderInterfaces(interfaces) {
            ...
            interfacesBody.innerHTML = interfaces.map(item => {
                ...
                return `<tr class="table-row border-b border-gray-200"><td class="px-4 py-2 font-mono text-xs">${item.name || '-'}</td><td class="px-4 py-2">${item.type || '-'}</td><td class="px-4 py-2">${state}</td></tr>`;
            }).join('');
        }
